...
// //employee state determin its animation and working
...
//   public float workPlaceUnlockTimer=3f;
...
//            Debug.Log("agent disable:" + agent.isActiveAndEnabled);
...
public class Employee : MonoBehaviour
{
    ...
    public TextMeshProUGUI remaningutyTimeText;
    public bool isPermanent;
    public bool isHiredForSession;
    float workPlaceUnlockTimer = 3f;
    public virtual void OnSpawn(EmployeeSavableData data, DepartmentsWorkPlace workPlace, float _servingTime, float _managerCommunicationDelay)
    {
        print("1B");
        workPlace.isOccupied = true;
        workPoint = workPlace.workPoint;
        restPoint = workPlace.restPoint;
        leavePoint = workPlace.leavingPoint;
        savableData = data;
        deptWorkPlaceRefs = workPlace;
        servingTime = _servingTime;
        managerCommunicationDelay = _managerCommunicationDelay;
        managerCommunicationDelayTimer = _managerCommunicationDelay;
        servingTimer = _servingTime;
        ...
        employeestate = (Employeestate)data.employeeState;
        // Get required components
        anim = GetComponent<Animator>();
        agent = GetComponent<NavMeshAgent>();
        ChangeEmployeeState(employeestate);

        if (isPermanent || isHiredForSession)
        {
            timerCanvas.SetActive(false);
        }
    ...
    public virtual void OnEndDuty()
    {
        ...
        EnableAgent();
        ChangeEmployeeState(Employeestate.Leaving);
      //  print("Altis" + leavePoint.position.x);
      //  print("Altis" + leavePoint.position.y);
      //  print("Altis" + leavePoint.position.z);
        agent.SetDestination(leavePoint.position);
        UnRegisterWithWorkManager();
    ...
    public virtual void Update()
    {
       // print("Beta ");
        ...
        if (employeestate == Employeestate.WaitingForWorkPlaceToUnlock)
        {
            ...
            if (workPlaceUnlockTimer <= 0)
            {
                if (EmployeeManager.Instance.IsWorkplaceUnlocked(departmentId, savableData.workPointIndex))
                {
                    ChangeEmployeeState(Employeestate.MovingToWorkPlace);
                }
                else if (EmployeeManager.Instance.isWorkPlaceFreeAndUnlockYet(departmentId, gameObject.GetComponent<Employee>()))
                {
                    ChangeEmployeeState(Employeestate.MovingToWorkPlace);
                }
                workPlaceUnlockTimer = 3f;
        ...
        if (employeestate == Employeestate.Waiting || employeestate == Employeestate.Rest || employeestate == Employeestate.Serving)
        {
           // print("111111111111111111111@@@@@@@@@@@@@@@@@@@@@@@@@");
            ...
            if (savableData.remDutyTime <= 0)
            {
            //    print("22222222222222222222@@@@@@@@@@@@@@@@@@@22");
                OnEndDuty();
                GameManager.instance.CallFireBase("EmpLeav_" + savableData.employeeType + "_" + savableData.employeeId);
                return;
            }
            int _remSec = Mathf.FloorToInt(savableData.remDutyTime);
            int hours = (_remSec / 60);
            int totalMinutes = _remSec % 60;

            int _days = hours / GameController.instance.dayCycleMinutes;

            int remHours = hours % GameController.instance.dayCycleMinutes;

            string timeString = string.Format("{0:00} : {1:00} : {2:00}", _days.ToString() + "D", remHours.ToString() + "H", totalMinutes.ToString() + "M");
            remaningutyTimeText.text = timeString;
        }

    }
    public void OnHireForSession()
    {
        isHiredForSession = true;
        if (savableData != null)
        {
            savableData.remDutyTime = 0f;
        }
    }
    /// <summary>
    /// Changes the Employee's state.
    ...
    public void DisableTimerCanvas()
    {
        if (isPermanent || isHiredForSession)
        {
            timerCanvas.SetActive(false);
    ...
    public void MakeEmployeeNonPermanenet()
    {
        isPermanent = false;
        savableData.remDutyTime = 0;
    }
